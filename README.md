...
## What it actually does
  ...
  - **Agent** -- hand it tools and let it run the whole task itself: `bash`, `python`, `web`, `browser`, `email`, `calendar`, `notes`, `tasks`, `memory`, `skills`, `image`, and any MCP server you add.<br>　<sub>built on [opencode](https://github.com/anomalyco/opencode) · MCP · web · files · shell · skills · memory · SSE streaming</sub>
    - *Small-context local models:* the agent prompt is tiered by the model's context window -- **tiny** (≤8K: 3 retrieved tools, only `bash`/`read_file`/`web_search`/`web_fetch` always on, no skill index), **small** (≤16K: 5 tools, trimmed always-on set, 8 skills), **medium** (≤32K: 8 tools, 20 skills), **full** (larger or unknown). Force a tier with the `agent_prompt_tier` setting (`auto` | `tiny` | `small` | `medium` | `full`); the chosen tier is reported in the `agent_prep` event and logged as `[prompt-budget]`.
  ...
  - **The Council** -- ask up to three models the same question at once and let the reviewer model synthesize consensus, disagreements, and a recommended answer.<br>　<sub>`/council <question>` · palette action · owner-scoped endpoint keys · outbound-URL check · `/api/council/ask`</sub>
  - **Today briefing** -- a collapsed card on the empty-chat screen composed from unread mail, today's calendar, pinned or due notes, and scheduled tasks, with an optional spoken summary.<br>　<sub>user-local day via `X-Tz-Offset` · pooled headers-only IMAP · 60s server cache · Read aloud via `/api/tts/synthesize` · `/api/briefing/today`</sub>
  - **Command palette** -- Ctrl+K searches chats, jumps to any tool, switches model, or opens Settings from one fuzzy list.<br>　<sub>actions · sessions · models · message search · listbox/combobox a11y · `search-chat.js` + `paletteItems.js`</sub>
  - **Agent floor** -- every agent run draws a small isometric floor above its tool timeline: a minifig walks between web, shell, files, browser and memory desks as tools fire.<br>　<sub>same iso family as the Paperclip Floor · one strip per turn · Appearance toggle · `chatFloor.js`</sub>
  - **Cockpit gauge** -- the composer shows the last response's tokens/sec and an honest context-window fill bar from the backend's `context_percent`.<br>　<sub>reads the `metrics` / `model_info` stream events · resets on session switch · `cockpit.js`</sub>
  - **Memory constellation** -- a dimmed force-directed graph of your memories behind the empty chat; typing brightens the related nodes.<br>　<sub>`/api/memory/graph` · masked center · stopword-aware matching, top 12 · Appearance toggle · `welcomeConstellation.js`</sub>
