    ...
    if channel == "email":
        try:
            ...
            if missing:
                ...
                msg["Subject"] = f"Reminder (Apollo): {_t}"
                msg["Date"] = _dt.now(_tz.utc).replace(tzinfo=None).strftime("%a, %d %b %Y %H:%M:%S +0000")
                msg["X-Apollo-Origin"] = "apollo-ui"
                msg["X-Apollo-Kind"] = "reminder"
                msg["X-Apollo-Ref"] = str(note_id)
