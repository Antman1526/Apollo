...
ATTACHMENTS_DIR = Path(os.environ.get("APOLLO_MAIL_ATTACHMENTS_DIR", str(DATA_DIR / "mail-attachments")))
...
def _init_scheduled_db():
    ...
    try:
        ...
        if "apollo_kind" not in cols:
            conn.execute("ALTER TABLE scheduled_emails ADD COLUMN apollo_kind TEXT")
...
_IMAP_TIMEOUT_SECONDS = _coerce_imap_timeout_seconds(os.environ.get("APOLLO_IMAP_TIMEOUT_SECONDS"))
...
class SendEmailRequest(BaseModel):
    ...
    # Internal marker for Apollo-generated mail (e.g. reminder, scheduled).
    apollo_kind: Optional[str] = None
