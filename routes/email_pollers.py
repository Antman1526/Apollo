...
async def _auto_summarize_pass_single(days_back: int = 1, account_id: str | None = None, progress_cb=None) -> str:
    ...
    try:
        ...
        since = (_utcnow() - _td(days=max(1, days_back))).strftime("%d-%b-%Y")
                        ...
                        if resp.ok:
                            ...
                            if summary:
                                ...
                                _c.execute("""
                                    ...
                                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                                """, (message_id, uid.decode() if isinstance(uid, bytes) else str(uid), _folder, subject, sender, summary, model, _utcnow().isoformat()))
                                _c.commit()
                                _c.close()
                    ...
                    try:
                        ...
                        if reply:
                            ...
                            _c.execute("""
                                ...
                                VALUES (?, ?, ?, ?, ?, ?)
                            """, (message_id, uid.decode() if isinstance(uid, bytes) else str(uid), _folder, reply, model, _utcnow().isoformat()))
                            _c.commit()
                            _c.close()
                    ...
                    try:
                        ...
                        _cc.execute(
                            ...
                            (message_id, uid.decode() if isinstance(uid, bytes) else str(uid),
                             _cal_run_count, _utcnow().isoformat())
                        )
                        _cc.commit()
                        _cc.close()
                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in Apollo (if public URL is configured).
                                    ...
                                    # HTML alternative with a clickable "Open in Apollo" button
                                    import html as _h
                                    ...
                                    outer_alert["Date"] = _utcnow().strftime("%a, %d %b %Y %H:%M:%S +0000")
                        ...
                        if not resp.ok:
                            ...
                            if parsed is not None:
                                ...
                                """, (message_id, account_owner or "", uid.decode(), subject, sender,
                                      ...
                                      spam_reason, moved_to, model, _utcnow().isoformat()))
                                _c.commit()
                                _c.close()
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "apollo_kind" if "apollo_kind" in cols else "'scheduled' AS apollo_kind"
        ...
        for r in rows:
            ...
            try:
                ...
                apollo_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-Apollo-Origin"] = "apollo-ui"
                outer["X-Apollo-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", apollo_kind or "scheduled")[:64]
                outer["X-Apollo-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `apollo-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `APOLLO_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `apollo-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("APOLLO_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `APOLLO_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (APOLLO_INPROCESS_POLLERS=0); "
            "drive `apollo-mail poll-scheduled` externally."
        )
        return
    import asyncio
