...
_HF_TOKEN_STATUS_SNIPPET = (
    'if [ -n "$HF_TOKEN" ]; then '
    'echo "[apollo] HF token: applied"; '
    'else '
    'echo "[apollo] HF token: NOT SET — gated/private models will be denied. '
    'Add one in Apollo Settings -> Cookbook -> HuggingFace Token."; '
    'fi'
)
...
def setup_cookbook_routes() -> APIRouter:
    ...
    async def generate_cookbook_ssh_key(request: Request):
        ...
        if not key_path.exists():
            ...
            proc = await asyncio.create_subprocess_exec(
                ssh_keygen, "-t", "ed25519", "-N", "", "-C", "apollo-cookbook", "-f", str(key_path),
    ...
    def _user_shell_path_bootstrap() -> list[str]:
        return [
            'APOLLO_USER_SHELL="${SHELL:-}"',
            'if [ -n "$APOLLO_USER_SHELL" ] && [ -x "$APOLLO_USER_SHELL" ]; then',
            '  APOLLO_USER_PATH="$("$APOLLO_USER_SHELL" -ic \'printf "__APOLLO_PATH__%s\\n" "$PATH"\' 2>/dev/null | sed -n \'s/^__APOLLO_PATH__//p\' | tail -n 1 || true)"',
            '  if [ -n "$APOLLO_USER_PATH" ]; then export PATH="$APOLLO_USER_PATH:$PATH"; fi',
            'fi',
        ]
    ...
    async def model_cached(request: Request, host: str | None = None, model_dir: str | None = None, ssh_port: str | None = None, platform: str | None = None):
        ...
        if host:
            ...
            # LOCAL scan: use sys.executable (the venv Python Apollo is already
    ...
    async def model_serve(request: Request, req: ServeRequest):
        ...
        if is_windows and remote:
            ...
            runner_lines = ["#!/bin/bash"]
            runner_lines.extend(_user_shell_path_bootstrap())
            runner_lines.append('APOLLO_PREFLIGHT_EXIT=""')
            # Put Apollo's own venv bin on PATH (local runs only) so the serve
            ...
            if "llama_cpp" in req.cmd or "llama-server" in req.cmd:
                ...
                runner_lines.append('    APOLLO_PREFLIGHT_EXIT=127')
                runner_lines.append('  fi')
                runner_lines.append('fi')
                ...
                runner_lines.append(f'APOLLO_OLLAMA_HOST={_bash_squote(_ollama_host)}')
                runner_lines.append(f'APOLLO_OLLAMA_PORT="{_ollama_port}"')
                runner_lines.append('APOLLO_OLLAMA_URL=""')
                runner_lines.append('for _ody_ollama_try in $(seq 1 20); do')
                runner_lines.append('  for _ody_ollama_port in "$APOLLO_OLLAMA_PORT" 11434; do')
                ...
                runner_lines.append('        APOLLO_OLLAMA_URL="$_ody_ollama_url"')
                runner_lines.append('        APOLLO_OLLAMA_PORT="$_ody_ollama_port"')
                runner_lines.append('        break 3')
                runner_lines.append('      fi')
                runner_lines.append('    done')
                runner_lines.append('  done')
                runner_lines.append('  [ "$_ody_ollama_try" -eq 1 ] && echo "[apollo] Waiting for an existing Ollama API on ports ${APOLLO_OLLAMA_PORT}/11434..."')
                runner_lines.append('  sleep 1')
                runner_lines.append('done')
                runner_lines.append('if [ -n "$APOLLO_OLLAMA_URL" ]; then')
                runner_lines.append('  if [ "$APOLLO_OLLAMA_PORT" != "' + _ollama_port + '" ]; then')
                runner_lines.append('    echo "[apollo] Selected Ollama port ' + _ollama_port + ' was not reachable; using running Ollama on port ${APOLLO_OLLAMA_PORT}."')
                runner_lines.append('  fi')
                runner_lines.append('  echo "[apollo] Ollama API ready on port ${APOLLO_OLLAMA_PORT}: ${APOLLO_OLLAMA_URL}"')
                runner_lines.append('  echo "[apollo] This task is monitoring an existing Ollama server; stopping it here will not stop an external Docker/system service."')
                runner_lines.append('  exec bash -i')
                runner_lines.append('fi')
                runner_lines.append('if ! command -v ollama &>/dev/null; then')
                runner_lines.append('  echo "ERROR: Ollama not found and no Ollama API is reachable on 127.0.0.1, localhost, or host.docker.internal (ports ${APOLLO_OLLAMA_PORT}/11434)."')
                ...
                runner_lines.append('  exec bash -i')
                runner_lines.append('fi')
                runner_lines.append('APOLLO_OLLAMA_URL="http://${APOLLO_OLLAMA_HOST}:${APOLLO_OLLAMA_PORT}"')
                if remote and _ollama_host in ("0.0.0.0", "::"):
                    runner_lines.append('echo "[apollo] WARNING: remote Ollama will bind to ${APOLLO_OLLAMA_HOST}:${APOLLO_OLLAMA_PORT} so Apollo can reach it from this host."')
                    runner_lines.append('echo "[apollo] Ollama has no built-in authentication; expose this only on a trusted LAN/VPN or provide an explicit OLLAMA_HOST with your own access controls."')
                runner_lines.append('echo "Starting ollama server on ${APOLLO_OLLAMA_HOST}:${APOLLO_OLLAMA_PORT}..."')
                runner_lines.append('OLLAMA_HOST="${APOLLO_OLLAMA_HOST}:${APOLLO_OLLAMA_PORT}" ollama serve')
                ...
                runner_lines.append('  APOLLO_PREFLIGHT_EXIT=1')
                runner_lines.append('fi')
    ...
    async def server_setup(request: Request, req: SetupRequest):
        ...
        if platform == "windows":
            ...
            setup_script = (
                'powershell -Command "'
                "New-Item -ItemType Directory -Force -Path $env:TEMP\\apollo-sessions | Out-Null; "
