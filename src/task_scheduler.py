...
HOUSEKEEPING_DEFAULTS = {
    ...
    "auto_distill_sessions": {"name": "Memory Auto-Distill",     "schedule": "cron",  "scheduled_time": None,    "cron_expression": "0 */6 * * *", "ship_paused": True, "legacy_names": []},
    "sync_memory_packs":    {"name": "Memory Sync",              "schedule": "cron",  "scheduled_time": None,    "cron_expression": "0 */4 * * *", "legacy_names": []},
    "suggest_skills_from_history": {"name": "Skill Suggestions",  "schedule": "cron",  "scheduled_time": None,    "cron_expression": "0 7 * * *", "legacy_names": []},
...
class TaskScheduler:
    ...
    async def _deliver_via_email(self, output: str, task, result: str):
        ...
        try:
            ...
            msg["X-Apollo-Origin"] = "apollo-ui"
            msg["X-Apollo-Kind"] = "task"
            msg["X-Apollo-Ref"] = str(task.id)
    ...
    async def _deliver_via_mcp(self, tool_name: str, task, result: str):
        ...
        args = {
            ...
            "body": result,
            "headers": {
                "X-Apollo-Origin": "apollo-ui",
                "X-Apollo-Kind": "task",
                "X-Apollo-Ref": str(task.id),
            },
        }
        if recipient:
