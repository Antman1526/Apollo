...
async def archive_inactive_sessions(session_manager, owner: Optional[str] = None) -> int:
    ...
    """
    cutoff_date = datetime.now(timezone.utc).replace(tzinfo=None) - timedelta(days=CleanupConfig.ARCHIVE_AFTER_DAYS)
    archived_count = 0
...
async def cleanup_old_sessions(session_manager, owner: Optional[str] = None) -> Tuple[int, float]:
    ...
    """
    cutoff_date = datetime.now(timezone.utc).replace(tzinfo=None) - timedelta(days=CleanupConfig.DELETE_AFTER_DAYS)
    deleted_count = 0
    space_freed = 0
...
async def get_cleanup_preview(owner: Optional[str] = None) -> Dict[str, Any]:
    ...
    """
    cutoff_archive = datetime.now(timezone.utc).replace(tzinfo=None) - timedelta(days=CleanupConfig.ARCHIVE_AFTER_DAYS)
    cutoff_delete = datetime.now(timezone.utc).replace(tzinfo=None) - timedelta(days=CleanupConfig.DELETE_AFTER_DAYS)
