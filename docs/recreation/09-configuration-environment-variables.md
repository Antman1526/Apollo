...
| `APOLLO_LLAMA_KV_CACHE` | `"q8_0"` | KV-cache precision for llama.cpp chat models (`q8_0` halves the memory a context window takes; `f16` is full precision). Fallback for the `local_model_kv_cache` setting. A launch that fails with `q8_0` retries with `f16` by itself. | `services/localmodels/config.py`, `server_manager.py` |
| `APOLLO_LLAMA_CONTEXT` | `"16384"` | Context window llama-server is launched with; fallback for the `local_model_context` setting (Settings → AI → Local Models → Context size, 8K–1M or `0` = auto via llama.cpp `--fit`). Always capped at the model's own limit read from its GGUF header / MLX config, so `1048576` means "as much as this model allows". A window that doesn't fit in memory retries with f16 KV, then auto. Apollo budgets prompts against the window actually served (`/props` `n_ctx`). | `services/localmodels/config.py`, `server_manager.py` |
