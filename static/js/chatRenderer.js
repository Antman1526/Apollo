...
// Pure model-metadata + text helpers live in a dependency-free leaf module so
// they can be unit-tested under Node. Imported here and re-exported below so
// this module's public API is unchanged.
import {
  MODEL_INFO, MODEL_PRICING, IMAGE_PRICING,
  shortModel, modelColor, getModelInfo, getModelCost,
  isLocalEndpoint, getImageCost, stripToolBlocks,
} from './modelMeta.js';
export {
  MODEL_INFO, MODEL_PRICING, IMAGE_PRICING,
  shortModel, modelColor, getModelInfo, getModelCost,
...
export function showWelcomeScreen() {
  const ws = document.getElementById('welcome-screen');
  const cc = document.getElementById('chat-container');
  ws?.classList.remove('hidden'); cc?.classList.add('welcome-active');
...
const _ACTION_RECENTS_KEY = 'apollo-msg-actions-recent';
 ...
 */
const _USER_ACTION_RECENTS_KEY = 'apollo-user-actions-recent';
    ...
    ctxRing.addEventListener('click', (e) => {
      ...
      if (compactBtn) {
        compactBtn.addEventListener('click', async (e) => {
          ...
          compactRole.textContent = 'Apollo';
...
export function addMessage(role, content, modelName, metadata) {
  try {
    ...
    var _roleText = role === 'user' ? 'You' : (isSlash || isCompacted) ? 'Apollo' : shortModel(resolvedModel);
