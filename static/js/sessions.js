...
let _sortMode = Storage.get('apollo-session-sort') || 'active'; // default to last active
...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'Apollo Chat';
...
const FOLDER_STATE_KEY = 'apollo-folder-state';
const FOLDER_ORDER_KEY = 'apollo-folder-order';
...
export async function selectSession(id, { keepSidebar = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to Apollo kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'Apollo Chat';
    }
    ...
    if (window.documentModule) {
      ...
      const meta = sessions.find(s => s.id === id);
      const shouldOpen = localStorage.getItem('apollo-doc-open-' + id) === '1';
...
export function setSortMode(mode) {
  _sortMode = mode || null;
  if (mode) Storage.set('apollo-session-sort', mode);
  else Storage.remove('apollo-session-sort');
  renderSessionList();
}
