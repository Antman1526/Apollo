...
import { buildVersionDiffSummary } from './document/versionHistory.js';
  ...
  async function _resolveComposeSendAccountId() {
    const activeAccountId = window.__apolloActiveEmailAccount || null;
  ...
  const _docOpenKey = (sessionId) => 'apollo-doc-open-' + sessionId;
  const _docMinimizedKey = (sessionId) => 'apollo-doc-minimized-' + sessionId;
    ...
    try {
      const res = await fetch(`${API_BASE}/api/email/draft`, {
        ...
        body: JSON.stringify({
          ...
          account_id: window.__apolloActiveEmailAccount || null,
        }),
      });
      const data = await res.json();
      if (data.success) {
  ...
  export function openPanel() {
    ...
    let _fontIdx = parseInt(localStorage.getItem('apollo-doc-fontsize') || '0', 10);
    ...
    function _applyDocFont() {
      ...
      localStorage.setItem('apollo-doc-fontsize', _fontIdx);
    }
    _applyDocFont();
  ...
  const _DOC_RECENTS_KEY = 'apollo-doc-actions-recent';
  ...
  function _saveSuggestionsToStorage() {
    ...
    if (data.length) {
      ...
      localStorage.removeItem('apollo-suggestions-' + activeDocId);
    }
  }
