#!/bin/bash
# Apollo — one-command quick start for macOS (Apple Silicon).
#
#   ./start-macos.sh
#
# Installs everything Apollo needs via Homebrew, sets up a local Python
...
# that's already done.
#
# Why native (not Docker): Cookbook serves models on whatever machine Apollo
...
# Shell overrides (APOLLO_PORT / APOLLO_HOST) take top priority, then .env
...
HOST="${APOLLO_HOST:-${APP_BIND:-127.0.0.1}}" # Set APP_BIND=0.0.0.0 in .env for LAN/Tailscale access.
PROBE_HOST="$HOST"
...
echo "▶ Apollo quick start for macOS"
...
if (exec 3<>"/dev/tcp/$PROBE_HOST/$PORT") 2>/dev/null; then
  ...
  echo "    APOLLO_PORT=7900 ./start-macos.sh"
  exit 1
fi
...
echo "▶ Preparing Apollo…"
APOLLO_SKIP_RUN_HINT=1 ./venv/bin/python setup.py
...
# APOLLO_NO_OPEN=1 (e.g. over SSH / headless).
POLLER_PID=""
if [ -z "$APOLLO_NO_OPEN" ] && command -v open >/dev/null 2>&1; then
  (
    for _ in $(seq 1 90); do
      if (exec 3<>"/dev/tcp/$PROBE_HOST/$PORT") 2>/dev/null; then
        printf '\n'
        printf '  ┌────────────────────────────────────────────┐\n'
        printf '  │  ✓ Apollo is ready — opening your browser  │\n'
        printf '  │     %-40s │\n' "$URL"
...
echo
echo "▶ Starting Apollo — it will open in your browser at $URL"
...
# a Node runtime when the Paperclip feature is enabled). No-op unless enabled.
