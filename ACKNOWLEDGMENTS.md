...
Apollo stands on the shoulders of a lot of open-source work. This file
...
Apollo is a renamed distribution of **[Odysseus](https://github.com/pewdiepie-archdaemon/odysseus)**,
created by **pewdiepie-archdaemon**. All of the original project's architecture,
features, and code are the work of the Odysseus author and its contributors;
Apollo only changes the project's name. Full credit and thanks to them for the
original work, which remains available under the MIT License (see
[`LICENSE`](LICENSE)).
...
- **[Tongyi DeepResearch](https://github.com/Alibaba-NLP/DeepResearch)** by
  **Alibaba-NLP / Tongyi Lab** — the multi-step deep-research agent pipeline.
  Copyright © Alibaba-NLP / Tongyi Lab. **Apache-2.0.** Adapted for Apollo's
...
and run alongside Apollo on `docker compose up`. They are not modified —
just composed.
...
Apollo talks to these over the network/API. They are **not** distributed
...
- **`caldav`** (Python lib) is **dual-licensed GPL-3.0-or-later OR Apache-2.0**.
  Apollo uses it under **Apache-2.0**, which is permissive and MIT-compatible.
...
Most of Apollo's code was written *with* AI models, not just by a human.
