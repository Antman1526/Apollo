...
def test_detect_system_propagates_unified_memory(monkeypatch):
    ...
    # Force the generic detection path: on a local Windows host detect_system
    # short-circuits into _detect_windows() before GPU probing, so the mocked
    # _detect_apple_silicon below would never be consulted.
